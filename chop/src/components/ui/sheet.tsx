...
interface SheetContentProps
  ...
  /** Hide the default top-right close control (use header/back + footer actions instead). */
  hideClose?: boolean;
}
...
>(({ side = "right", className, children, hideClose = false, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {!hideClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 flex size-11 items-center justify-center rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
